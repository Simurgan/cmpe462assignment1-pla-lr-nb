...
class LogisticRegression:
    ...
    def scale_features(self):
        for i in range(self.num_features):
            ...
            for d in range(self.N):
                self.data[d][i] = (self.data[d][i] - min_val) / (max_val - min_val + eps)
    ...
    def split_data(self, training_ratio):
        ...
        self.training_labels[self.training_labels == "B"] = -1.0
        self.training_labels[self.training_labels == "M"] = 1.0
        self.training_labels = self.training_labels.reshape((self.N_training, 1)).astype(np.float64)

        self.test_labels = test_split.T[-1]
        self.test_labels[self.test_labels == "B"] = -1.0
        self.test_labels[self.test_labels == "M"] = 1.0
